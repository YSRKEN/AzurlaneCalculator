	...
	class CalcProbPageViewModel : INotifyPropertyChanged
	{
		...
		public CalcProbPageViewModel() {
			...
			Output1 = DropProb.CombineLatest(DropCount, (prob, count) => {
				...
				output += $"\n50,70,90,95,99％成功：\n";
				double per50Count = Math.Ceiling(CalcProb.CalcBernoulliProb(prob_, 0.5));
				double per70Count = Math.Ceiling(CalcProb.CalcBernoulliProb(prob_, 0.7));
				double per90Count = Math.Ceiling(CalcProb.CalcBernoulliProb(prob_, 0.9));
				double per95Count = Math.Ceiling(CalcProb.CalcBernoulliProb(prob_, 0.95));
				double per99Count = Math.Ceiling(CalcProb.CalcBernoulliProb(prob_, 0.99));
				output += $"{per50Count},";
				output += $"{per70Count},";
				output += $"{per90Count},";
				output += $"{per95Count},";
			...
			Output2 = DropCount2.CombineLatest(
				SuccessDropCount, DropProb2, (all, success, prob) => {
					...
					double[] ci = CalcProb.ClopperPearsonMethod(all, success, 0.95M);
					output += $"\n95％信頼区間：{Math.Round(100.0 * ci[0], 1)}～{Math.Round(100.0 * ci[1], 1)}％";
					output += $"\np値：{Math.Round(CalcProb.SterneMethod(all, success, prob), 6)}";
					return output;
				}
			).ToReadOnlyReactiveProperty();
		}
