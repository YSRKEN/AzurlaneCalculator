			...
			using (var reader = new StreamReader(stream, Encoding.UTF8)) {
				...
				while ((line = reader.ReadLine()) != null) {
					...
					var mapExp = new MapExp {
						Area = int.Parse(temp[0]),
						Stage = int.Parse(temp[1]),
						EnemyExpS = int.Parse(temp[2]),
						EnemyExpM = int.Parse(temp[3]),
						EnemyExpL = int.Parse(temp[4]),
						EnemyExpB = int.Parse(temp[5]),
						EnemyCount = int.Parse(temp[6]),
					};
					output.Add(mapExp);
				}
			}
